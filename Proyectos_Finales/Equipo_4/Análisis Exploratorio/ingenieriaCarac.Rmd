...
```{r}
datos <- read.csv("motorolalimpios.csv")
...
```{r}
#left_join() Combinar 2 datasets. Manteniendo las observaciones de la tabla de origen
#merge() Combinar grupos de datos
```

```{r}
#Son variables que seran manipuladas de una manera que requiere estar en ese formato
```
